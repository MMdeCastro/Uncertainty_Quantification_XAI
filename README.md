...
Podemos cerciorarnos de que esto no nos pasa incorporando a nuestro modelo ya entrenado ciertas herramientas para que pueda indicarnos cuándo su predicción es fiable y cuándo no. Estas herramientas matemáticas (ya desarrolladas como librerías de Python) explican en qué se fijó el modelo para producir sus predicciones (gracias a métodos de explicabilidad o [XAI](https://en.wikipedia.org/wiki/Explainable_artificial_intelligence), como los SHAP values, LIME,...) o nos proveen de probabilidades calibradas y rangos de certidumbre que incluyen con certeza el valor verdadero en lugar de predicciones heurísticas y puntuales para que el modelo pueda avisarnos cuando no debemos confiar en su prediccíon (gracias a métodos de cuantificación de incertidumbre o [UQ](https://en.wikipedia.org/wiki/Uncertainty_quantification), como los Conformal Predictors). Y todas son herramientas post-hoc (no hay que volver a entrenar el modelo), model-agnóstic (sirven para cualquier modelo), y muy ligeras (se implementan con pocas líneas de código y se ejecutan muy rápido).
